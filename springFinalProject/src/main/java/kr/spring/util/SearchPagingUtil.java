...
public class SearchPagingUtil {
	 ...
	 * */
	public SearchPagingUtil(int currentPage, int totalCount, int rowCount,
	...
	public SearchPagingUtil(String keyfield, String keyword, int currentPage, int totalCount, int rowCount,
			int pageCount,String pageUrl) {
	...
	public SearchPagingUtil(String keyfield, String keyword, int currentPage, int totalCount, int rowCount,
		...
		if (currentPage > pageCount) {
			if(keyword==null){//검색 미사용시
				...
				pagingHtml.append("<a href="+pageUrl+"&keyfield="+keyfield+"&keyword="+keyword+"&" + addKey + "PageNum="+ (startPage - 1) + ">");
			}
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (i == currentPage) {
				...
				if(keyword==null){//검색 미사용시
					...
					pagingHtml.append("&nbsp;<a href='"+pageUrl+"&keyfield="+keyfield+"&keyword="+keyword+"&" + addKey + "PageNum=");
				}
				pagingHtml.append(i);
				pagingHtml.append("'>");
				pagingHtml.append(i);
		...
		if (totalPage - startPage >= pageCount) {
			if(keyword==null){//검색 미사용시
				...
				pagingHtml.append("<a href="+pageUrl+"&keyfield="+keyfield+"&keyword="+keyword+"&" + addKey + "PageNum="+ (endPage + 1) +">");
			}
